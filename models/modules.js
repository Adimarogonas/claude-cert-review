        ...
        questions: [
          {
            ...
            correct: 3,
            explanation: "If each subagent executed correctly within its assigned scope, the problem is the scope assigned to them — that's the coordinator's decomposition. Options B, C, and D incorrectly blame downstream agents that performed their assigned work correctly. The coordinator is responsible for ensuring comprehensive coverage across the full topic space.",
          },
          {
            id: "m1.2b",
            ...
            options: [
              "The synthesis agent will receive inconsistent inputs when some agents return empty results",
        ...
        questions: [
          {
            ...
            correct: 1,
            explanation: "A programmatic prerequisite is deterministic — it blocks the downstream call regardless of what the model decides. Options B and C rely on probabilistic LLM compliance, which the 12% failure rate demonstrates is insufficient. Option D addresses tool availability, not tool ordering, which is the actual problem.",
          },
          {
            id: "m1.4b",
            ...
            correct: 2,
            explanation: "Structured handoff protocols must include customer details, root cause analysis, and recommended actions — human agents need enough context to continue without access to the full conversation transcript. Confidence scores (B) are unreliable proxies. A raw transcript (C) is often too verbose and hard for humans to parse quickly. A tool call list (D) is not the primary need.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            correct: 3,
            explanation: "PostToolUse hooks intercept tool results and transform them before the model sees them — the ideal solution for normalizing output formats. This is deterministic and doesn't require model compliance (B). PreToolUse (C) fires before execution, not after, and cannot modify results. Updating each tool (D) may not be feasible for third-party MCP servers.",
          },
          {
            id: "m1.5b",
            ...
            options: [
              "Move the refund limit rule to the very beginning of the system prompt for maximum salience",
            ...
            correct: 1,
            explanation: "A PreToolUse hook reads the actual refund amount at runtime and deterministically reroutes — the model cannot bypass it. Prompt position (B) and few-shot examples (C) are both probabilistic; the scenario already demonstrates prompt-based approaches fail. tool_choice: 'any' (D) controls whether a tool is called at all, not what happens based on parameter values.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            correct: 0,
            explanation: "The root cause is attention dilution from processing too many files simultaneously. Per-file passes ensure each file gets focused analysis; a separate integration pass catches cross-file issues. A larger context window (B) doesn't solve attention quality degradation. Three independent passes on all 14 files (C) would require consensus, suppressing real single-instance bugs. Option D shifts burden to developers without fixing the review architecture.",
          },
          {
            id: "m1.6b",
            ...
            correct: 2,
            explanation: "An open-ended investigation on an unknown codebase requires dynamic decomposition — what to test and in what order depends on what the structure analysis reveals. A fixed sequential pipeline (B, D) works when the steps are predictable upfront, but 'unknown dependencies' means the approach must adapt. A single-pass (C) on a 50K-line codebase will produce poor results due to context limits and attention dilution.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            correct: 3,
            explanation: "When prior tool results are stale (files have changed), resuming with those stale results in context leads to incorrect reasoning. Starting fresh with a structured summary of what was learned plus specific information about what changed is more reliable. Re-reading all files (B) within the resumed session still starts from a stale context baseline. fork_session (C) branches from the same stale context. Models cannot detect external file changes (D).",
          },
          {
            id: "m1.7b",
            ...
            correct: 1,
            explanation: "fork_session is designed exactly for this: divergent exploration from a shared baseline. Two independent sessions (C) waste time repeating the shared codebase analysis. Using --resume for both strategies (B) overwrites the same session context rather than branching it. Sequential investigation (D) doesn't allow genuine comparison of approaches explored under the same initial conditions.",
          },
        ],
      },
    ],
        ...
        questions: [
          {
            ...
            correct: 0,
            explanation: "Tool descriptions are the primary mechanism for tool selection. Minimal descriptions cause misrouting; expanding them with examples and clear boundaries directly addresses the root cause. Few-shot examples (B) add token overhead without fixing the underlying description problem. A routing layer (C) is over-engineered and bypasses the model's natural language understanding. Consolidating tools (D) is a valid architectural choice but more complex than a description fix and may not be the right first step.",
          },
          {
            id: "m2.1b",
        ...
        questions: [
          {
            ...
            correct: 3,
            explanation: "Structured error metadata — errorCategory, isRetryable, and a descriptive message — gives the agent the information to make intelligent recovery decisions: retry (transient), reformulate (validation), escalate (business), or skip (permission). Generic messages (B) and empty result arrays (C) hide the error type. Propagating exceptions (D) may terminate the entire workflow unnecessarily.",
          },
          {
            id: "m2.2b",
        ...
        questions: [
          ...
          },
          {
            ...
            options: [
              ...
              "No tool_choice setting is needed; adding more examples to the system prompt will ensure the model calls the tool",
            ],
            correct: 2,
            explanation: "tool_choice: 'any' guarantees the model calls at least one tool, eliminating the risk of conversational text responses. 'auto' (B) is the problem — it allows text returns. Forced specific tool (C) works when document type is known and the right tool can be specified; for unknown document types with multiple potential extraction tools, 'any' is more appropriate. System prompt examples (D) are probabilistic.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            correct: 0,
            explanation: "Grep is for content search — finding where a specific string or pattern appears inside files. Searching for all files that import formatCurrency means searching file contents for that string, which is exactly what Grep does. Glob (B) matches file paths/names, not file contents. Read (C) would require reading every file individually, which is very inefficient. Bash find (D) is less appropriate than the dedicated Grep tool.",
          },
          {
            id: "m2.5b",
            ...
            correct: 2,
            explanation: "When Edit fails due to non-unique anchor text, the documented fallback is Read + Write: read the full file, make the modification programmatically, and write the entire updated file. Using Grep to find the location and retrying Edit (B) with more context is a reasonable alternative but not the canonical fallback pattern. Bash/sed (C) is fragile for complex edits. Option D describes the same Edit retry approach as B.",
          },
        ],
      },
      {
        id: "2.6",
        title: "Use Jira MCP to integrate issue tracking into agentic workflows",
        knowledge: [
          "Core Jira MCP tools: search_issues (JQL queries), create_issue, update_issue, get_issue, add_comment, list_projects, get_transitions, and transition_issue",
          "Jira statuses are workflow-controlled: you cannot set the status field directly via update_issue — you must call get_transitions to retrieve valid transitions for the current status, then call transition_issue with the chosen transition ID",
          "JQL (Jira Query Language) is the search mechanism — supports filtering by project, status, assignee, labels, sprint, text matches, and date ranges; prefer a targeted JQL search over repeated get_issue calls for bulk discovery",
          "API tokens must never be hardcoded in .mcp.json — use ${JIRA_API_TOKEN} (or equivalent env var expansion) so secrets stay out of version control",
          "MCP resource endpoints can expose prebuilt issue catalogs (e.g., all open bugs in a project) to reduce exploratory get_issue calls before triage or planning",
        ],
        skills: [
          "Search with JQL before calling create_issue to check whether a matching issue already exists and avoid filing duplicates",
          "Move issues through workflow states by calling get_transitions (to discover valid next states) followed by transition_issue — never by updating the status field directly",
          "Configure Jira API authentication via environment variable expansion in .mcp.json; place the config in project .mcp.json for team-shared access",
          "Use MCP resource endpoints to load project issue summaries in bulk rather than looping over individual get_issue calls",
        ],
        gotchas: [
          "Calling update_issue with a status field value and expecting the issue to move states — Jira ignores direct status field writes; transitions are mandatory",
          "Creating issues without searching first — agents operating in active projects frequently create duplicates when they skip the JQL pre-check",
          "Hardcoding the API token or base URL in .mcp.json — these get committed to version control; always use environment variable references",
          "Discovering open issues with repeated get_issue calls in a loop when a single JQL search_issues call returns the same result set in one round trip",
        ],
        questions: [
          {
            id: "m2.6a",
            text: "An agent is asked to file a Jira bug for a reported 'login timeout on mobile devices.' What should it do before calling create_issue?",
            options: [
              "Confirm the project key by calling list_projects and inspecting the returned project list",
              "Retrieve the project's active sprint with get_sprint so the issue can be filed against it",
              "Run a JQL search for existing login timeout issues to avoid creating a duplicate report",
              "Call get_issue_types to verify that Bug is a valid type in the selected project",
            ],
            correct: 2,
            explanation: "Searching with JQL before creating prevents duplicate issues — a common problem in active projects. list_projects (A) is useful for discovering project keys but doesn't prevent duplicates. get_sprint (B) is useful for sprint assignment but is not the first priority. get_issue_types (D) is a setup concern, not a per-issue workflow step.",
          },
          {
            id: "m2.6b",
            text: "A shared Jira MCP server is configured in the project's .mcp.json for the whole team. How should the Jira API token be supplied?",
            options: [
              "Use ${JIRA_API_TOKEN} in .mcp.json; each developer exports the variable in their local shell",
              "Hardcode the raw token directly in .mcp.json so all team members authenticate with a shared credential",
              "Store the token in CLAUDE.md under a credentials section so the agent reads it at runtime",
              "Add the token value to ~/.claude.json so it stays personal and teammates don't share access",
            ],
            correct: 0,
            explanation: "Environment variable expansion (${JIRA_API_TOKEN}) keeps secrets out of version control while letting every team member supply their own token via their local shell environment. Hardcoding (B) commits a live credential to git. CLAUDE.md (C) is not an authentication mechanism for MCP servers. ~/.claude.json (D) is user-scoped — it would give only that developer access, not the team.",
          },
          {
            id: "m2.6c",
            text: "An agent calls update_issue to set the 'status' field to 'In Review', but the status does not change and no error is returned. What is the most likely cause?",
            options: [
              "The agent needs a write:jira-work OAuth scope that grants permission to modify status fields directly",
              "The update_issue tool requires a version bump field alongside any status field change",
              "The issue must be assigned to a user before update_issue accepts status modifications",
              "Jira statuses are workflow-controlled; the agent must call get_transitions then transition_issue",
            ],
            correct: 3,
            explanation: "Jira's REST API (and by extension the MCP tools that wrap it) does not allow setting status via a direct field update. Status changes require a workflow transition: call get_transitions to retrieve valid next states for the issue, then call transition_issue with the appropriate transition ID. The other options describe plausible-sounding but incorrect constraints.",
          },
          {
            id: "m2.6d",
            text: "Before triaging work, an agent loops through every open ticket by calling get_issue individually, resulting in dozens of sequential API calls. Which Jira MCP capability reduces this overhead?",
            options: [
              "Set tool_choice to 'any' so the agent batches multiple get_issue calls in a single MCP request",
              "Use a Jira MCP resource endpoint that exposes a prebuilt issue summary catalog for the project",
              "Add a prompt cache directive so repeated get_issue responses are stored for the session",
              "Configure a webhook listener in .mcp.json so Jira pushes issue updates to the agent proactively",
            ],
            correct: 1,
            explanation: "MCP resource endpoints expose prebuilt content catalogs — such as all open issues in a project — that the agent can load in a single call instead of issuing individual get_issue requests. tool_choice (A) controls which tool the model calls, not how many API calls are made. Prompt caching (C) reduces token costs, not API round trips. Webhooks (D) are a push model that requires server-side infrastructure outside the MCP server scope.",
          },
        ],
      },
    ],
        ...
        questions: [
          ...
          },
          {
            ...
            options: [
              ...
              "Split into topic-specific files in .claude/rules/ (testing.md, api-conventions.md, etc.)",
            ...
            correct: 1,
            explanation: ".claude/rules/ is designed for this: modular, topic-specific rule files that can be path-scoped to load only when relevant. Compressing by removing examples (B) reduces clarity without fixing the organization problem. Subdirectory CLAUDE.md files (C) are directory-bound and can't easily handle conventions that apply to file types spread across the codebase. Moving sections to user-level (D) removes them from version control and other team members.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            options: [
              ...
              "In the CLAUDE.md file at the project root under a 'commands' section",
              "In .claude/config.json with a 'commands' array",
            ],
            correct: 0,
            explanation: "Project-scoped custom slash commands belong in .claude/commands/ in the repository. These files are version-controlled and automatically available to all developers when they clone or pull the repo. ~/.claude/commands/ (B) is user-scoped and not shared. CLAUDE.md (C) is for project context and instructions, not command definitions. .claude/config.json with a commands array (D) is not a valid Claude Code configuration mechanism.",
          },
          {
            id: "m3.2b",
            ...
            correct: 2,
            explanation: "context: fork runs the skill in an isolated sub-agent context. The skill's output stays in the sub-agent's context and only the final summary/result is returned to the main conversation, preventing context pollution. allowed-tools (B) restricts which tools the skill can call, not the output volume. argument-hint (C) is for prompting users for parameters. Moving to user-scope (D) makes it unavailable to the team.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            correct: 3,
            explanation: "A path-scoped rule with paths: ['**/*.test.*'] applies conventions to all test files regardless of their location in the directory tree. CLAUDE.md per directory (B) would require maintaining the same conventions in many places and is directory-bound, not file-type-bound. Root CLAUDE.md (C) loads the conventions even when editing non-test files, wasting context. A skill (D) requires manual invocation and won't automatically apply.",
          },
          {
            id: "m3.3b",
        ...
        questions: [
          ...
          },
          {
            ...
            options: [
              ...
              "Direct execution — the problem, location, and fix are all well-understood; plan mode overhead is not justified",
              "Start a new session with a structured summary of the bug before attempting the fix",
            ],
            correct: 2,
            explanation: "Direct execution is appropriate for simple, well-scoped changes with a clear fix. A single null check on a known line in a known file is the canonical direct execution case — the problem, location, and solution are all clear. Plan mode (B, C) adds architectural exploration overhead that provides no value when the fix is already known. The Explore subagent (D) is for isolating verbose discovery phases, not for simple fixes with clear scope.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            correct: 3,
            explanation: "Concrete input/output examples are the most effective technique when prose descriptions are interpreted inconsistently. Three failed iterations indicate that the description 'more readable' is ambiguous to the model. Examples eliminate this ambiguity by showing exactly what's expected. More detailed prose (B) may still be interpreted inconsistently. Plan mode (C) is for architectural scope, not format clarification. The interview pattern (D) is useful before implementation in unfamiliar domains, not for clarifying well-understood output format requirements.",
          },
          {
            id: "m3.5b",
            ...
            correct: 1,
            explanation: "When fixes interact (A and B), they should be addressed together in a single message — fixing A independently may break or complicate the B fix. When issues are independent (C: an unrelated typo), they can be addressed sequentially. This is the canonical pattern: interacting problems in one message, independent problems sequentially.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            correct: 3,
            explanation: "When a high false-positive category erodes trust in the entire tool, temporarily disabling it while improving its criteria restores trust in the accurate categories. Instructions like 'only report high-confidence findings' (B) and 'be more conservative' (C) are vague confidence-based filters that the exam guide explicitly identifies as ineffective compared to categorical criteria. Retraining (D) is out of scope for the exam.",
          },
          {
            id: "m4.1b",
      ...
      },
      {
        ...
        questions: [
          ...
          },
          {
            ...
            correct: 2,
            explanation: "Few-shot examples demonstrating extraction from varied document structures allow the model to generalize its extraction approach to the document type it encounters. The exam guide specifically cites 'inline citations vs bibliographies' as the case where few-shot examples are most effective. Separate prompts with classification (B) adds complexity and a classification failure mode. Instructions (C) are already not achieving the goal. Postprocessing (D) adds a fragile pipeline step for a problem solvable in the prompt.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            correct: 3,
            explanation: "This is the key distinction: strict JSON schemas guarantee syntactic validity (fields present, correct types, no malformed JSON) but cannot prevent semantic errors like line items that don't sum to the total, or values placed in the wrong fields. The fix requires post-extraction validation logic (e.g., calculating and comparing totals). The schema is working correctly (B is wrong). Option C incorrectly describes tool_use's function. D contradicts the question's premise that JSON is valid.",
          },
          {
            id: "m4.3b",
            ...
            correct: 1,
            explanation: "When a schema has required fields for information that may not exist in the source, the model fabricates values to satisfy the schema rather than return null. Making the field optional/nullable (e.g., 'publication_date': { 'type': ['string', 'null'] }) allows the model to return null when the information isn't available, which is the correct behavior. JSON schema validation (B) is a client-side concern; the model will attempt to provide the field. The Claude API doesn't auto-treat required fields as optional (D).",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            options: [
              "No — the info is absent from the document, so retries just push the model to fabricate a value",
          ...
          },
          {
            ...
            options: [
              ...
              "Retry with the specific error: 'The date must be YYYY-MM-DD; reformat March 15, 2024 to 2024-03-15'",
              "Use postprocessing to convert the date format rather than retrying",
            ],
            correct: 2,
            explanation: "Retry-with-error-feedback works best when the error is specific and actionable. Including the exact failed value and the exact expected format gives the model clear information for self-correction. An unchanged retry (B) may produce a different format but not necessarily the correct one. A vague error message (C) doesn't tell the model what went wrong. Postprocessing (D) could work for date formatting specifically but doesn't address the broader principle, and the question asks about retry approach.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            correct: 3,
            explanation: "The Message Batches API's up to 24-hour processing window with no guaranteed SLA makes it unsuitable for blocking pre-merge checks where developers need results promptly. The overnight report is the ideal use case: latency-tolerant, overnight generation, results reviewed the next morning. Option B is wrong because relying on 'often faster' completion is unacceptable for blocking workflows. Option C loses cost savings unnecessarily. Option D adds complexity without addressing the fundamental SLA mismatch.",
          },
          {
            id: "m4.5b",
            ...
            correct: 0,
            explanation: "custom_id fields are designed exactly for this: correlating request/response pairs so failed documents can be identified and selectively resubmitted with modifications (chunking oversized documents). Resubmitting all 500 (B) wastes cost and time on the 453 already-successful extractions. Accepting failures (C) may be acceptable in some cases but doesn't address the question of how to handle them. Switching to synchronous (D) loses the 50% cost savings across all successful documents.",
          },
        ],
      },
      {
        ...
        questions: [
          ...
          },
          {
            ...
            correct: 2,
            explanation: "Per-file passes ensure each file receives focused analysis at consistent depth; a separate integration pass then examines cross-file concerns. Larger context windows (B) don't solve attention quality degradation — the model still focuses more on earlier context. Three passes on all 20 files (C) requires consensus for flagging, which would suppress bugs that only appear in one pass. Splitting PRs (D) shifts burden to developers without fixing the underlying review architecture.",
          },
        ],
      },
    ],
        ...
        questions: [
          {
            ...
            correct: 3,
            explanation: "A persistent case facts block — containing specific amounts, dates, and key case details — sits outside the summarized history and is included in every prompt. This prevents progressive summarization from condensing specific values into vague approximations. Disabling compression entirely (B) is often not possible and burns context on irrelevant details. Larger context windows (C) delay but don't prevent the problem. Including in the latest message (D) is a workaround that breaks the normal conversation flow.",
          },
          {
            id: "m5.1b",
            ...
            correct: 1,
            explanation: "Trimming tool outputs to only relevant fields before they accumulate prevents irrelevant data from consuming context tokens disproportionately. Including all fields (B) fills context with noise and may push important information into middle positions. Prose summarization (C) risks the same condensation problem as progressive summarization. On-demand retrieval (D) adds complexity and additional tool call overhead.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            correct: 0,
            explanation: "Explicit requests for human agents must be honored immediately — this is a distinct escalation trigger separate from case complexity. Attempting to resolve first (B) and trying to convince the customer the bot can help (C) override an explicit preference, which damages trust. Sentiment analysis (D) is an unreliable proxy and irrelevant when the customer's preference is explicitly stated.",
          },
          {
            id: "m5.2b",
            ...
            correct: 2,
            explanation: "The agent is inverting escalation logic: escalating standard cases and handling exceptions. The root cause is unclear escalation decision boundaries. Adding explicit criteria with few-shot examples directly addresses this. Self-reported confidence (B) is an unreliable proxy — the agent may already be confident about the wrong decisions. A classifier (C) is over-engineered when prompt optimization hasn't been tried. Sentiment analysis (D) doesn't correlate with case complexity.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            correct: 3,
            explanation: "Structured error context gives the coordinator the information it needs for intelligent recovery: it knows what failed, what was attempted, what partial results exist, and what alternatives to try. Empty results with no flag (B) silently suppress the error and the coordinator cannot distinguish failure from 'no results.' Generic status (C) hides the context needed for recovery decisions. Terminating the entire workflow (D) is the anti-pattern of abandoning work when partial results could still be used.",
          },
          {
            id: "m5.3b",
        ...
        questions: [
          {
            ...
            correct: 0,
            explanation: "Context degradation occurs when verbose exploration output fills the context window — the model starts reasoning from compressed/approximated context rather than specific earlier findings. Subagent delegation isolates verbose output while the main agent maintains high-level coordination. /compact (B) is a tool that can help but doesn't address the underlying architectural issue. Restarting (C) loses all accumulated findings. Model version (D) is not the right lever for context management.",
          },
          {
            id: "m5.4b",
            ...
            correct: 2,
            explanation: "Subagents don't automatically inherit context from prior phases. Summarizing Phase 1 findings to a scratchpad file and injecting that summary into Phase 2 subagent prompts provides the necessary dependency context without requiring context sharing. Shared context (B) defeats the purpose of subagent isolation. Completing both phases in the same session (C) leads to the context degradation problem described in the previous question. Re-running Phase 1 per subagent (D) is wasteful and defeats the multi-phase architecture.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            options: [
              ...
              "Segment accuracy by document type and field — the 97% average may mask 60% on specific segments",
            ],
            correct: 3,
            explanation: "Aggregate metrics can hide segment-level failures. A 97% overall accuracy can coexist with 40–60% accuracy on specific document types — automating those extractions would produce systematic errors. Segmentation by document type and field is the critical analysis before automation. Self-reported confidence (B) is not reliably calibrated — it's a model estimate, not a validated accuracy metric. Scale confirmation (C) doesn't reveal segment failures hidden in the aggregate. The 3% threshold (D) may be acceptable on average but unacceptable for specific high-stakes fields.",
          },
        ],
      },
      {
        ...
        questions: [
          {
            ...
            correct: 0,
            explanation: "Source attribution is lost during summarization/synthesis when subagents don't preserve claim-source mappings. The fix is requiring subagents to output structured data including source URL, document name, and relevant excerpts alongside each claim, and requiring the synthesis agent to preserve these mappings through synthesis. A lookup tool (B) can't recreate attribution that was never captured. A central registry (C) is a valid alternative architecture but adds coordination overhead. Generating citations from training knowledge (D) is hallucination.",
          },
          {
            id: "m5.6b",
            ...
            options: [
              ...
              "Report both values with source attribution and annotate the conflict",
              "Exclude conflicting statistics from the report to maintain accuracy",
            ],
            correct: 2,
            explanation: "When credible sources conflict, the correct approach is to preserve both values with attribution and annotate the conflict rather than arbitrarily selecting one. Conflicting statistics often reflect different methodologies, sample populations, or time periods — context that is valuable to the reader. Selecting the more recent (B) or averaging (C) both discard information and misrepresent the state of knowledge. Excluding (D) presents an incomplete picture of what the sources say.",
          },
        ],
      },
    ],
